...
SET @idsbj := "109007" COLLATE utf8_unicode_ci;
SET @formtype := 1;
SET @sep := ';';
SET @sepreplacement := '|';
...
SELECT
  ...
  @texte := IF(fr.id IS NOT NULL, REPLACE(@text COLLATE utf8_unicode_ci, @sep, @sepreplacement), @blank)                           AS texte,
  @safeval := IF(fr.id IS NOT NULL, REPLACE(fr.val COLLATE utf8_unicode_ci, @sep, @sepreplacement), @blank)                        AS safeval,
  @safeprecis := IF(fr.id IS NOT NULL, REPLACE(fr.resp_precision COLLATE utf8_unicode_ci, @sep, @sepreplacement), @blank)          AS precis,
